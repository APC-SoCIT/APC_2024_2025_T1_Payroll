    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                Payroll Item for
                for <Link class="underline hover:text-gray-600" :href="route('account.get', targetAccount.id)">{{ targetAccount.name }}</Link>
                for <Link class="underline hover:text-gray-600" :href="route('cutoff.get', payrollItem.payroll_period.id)">{{ useFormat(payrollItem.payroll_period.end_date) }}</Link>
            </h2>
        </template>
        ...
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
                <div class="p-4 sm:p-8 space-x-4 bg-white shadow sm:rounded-lg">
                    <SecondaryButton><Link :href="route('account.get', targetAccount.id)">Edit account</Link></SecondaryButton>
                    <SecondaryButton><Link :href="route('cutoff.get', payrollItem.payroll_period.id)">Reschedule cutoff</Link></SecondaryButton>
                </div>
            </div>
